...
def return_edges():
    data = pd.read_csv("input/TRAIN_SET_SAMPLED.csv") #remove NEG later !!!
...
def return_features_labels():
    feature = "BERT"
    data = pd.read_csv("input/features/"+feature+".csv") #labels.csv
...
train_hetero_graph.nodes['judgement'].data['feature'] = features
