...
res_csv = pd.read_csv("RESULT.csv")
    ...
    for epochs in [400,600,800]:
        for sampling_rate in [1,2]:
            ...
            with torch.no_grad():
                ...
                row = [feature,epochs,sampling_rate,score]
                len_df = len(res_csv)
                res_csv.loc[len_df] = row
                res_csv.to_csv("RESULT.csv",index=False)
